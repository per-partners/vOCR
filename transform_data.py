...
# #     # return bool(re.fullmatch(r"^[0-9]+$", claim_id))
# #     return bool(re.fullmatch(r"^[NC-0-9]+$", claim_id))
# #     # return bool(re.fullmatch(r"^[a-zA-Z0-9]+$", claim_id))

# def is_valid_claim_id(claim_id: str) -> bool:
#     return bool(re.fullmatch(r"^\d+$", claim_id) or re.fullmatch(r"^NC\d+$", claim_id))
...
#     dataset_root = os.path.join(parent_dir, "processed_dataset_2")
...
def check_for_valid_claim_id(claim_id: str) -> bool:
    ...
    is_valid = bool(re.fullmatch(r"^\d+$", claim_id) or re.fullmatch(r"^NC\d+$", claim_id))
    if not is_valid:
...
def duplicate_and_split_dataset(parent_dir, num_duplicates=50, train_size=0.7, seed=42):
    ...
    logger.info(f"Starting dataset processing with parameters: train_size={train_size}, seed={seed}")
    
    images_dir = os.path.join(parent_dir, "images")
    labels_dir = os.path.join(parent_dir, "annotation")
    dataset_root = os.path.join(parent_dir, "processed_dataset")
    
    logger.info(f"Input directories: Images: {images_dir}, Labels: {labels_dir}")
    ...
    image_with_annotations = [img for img in image_names if os.path.exists(os.path.join(labels_dir, f"{img}.json"))]
    logger.info(f"Found {len(image_with_annotations)} images with matching annotation files")
    
    if len(image_with_annotations) < len(image_names):
        logger.warning(f"Missing annotations for {len(image_names) - len(image_with_annotations)} images")
    ...
    logger.info(f"Split dataset: {len(train_images)} training images, {len(valid_images)} validation images")
    ...
    for split, split_name in tqdm(
        ...
        for image_name in tqdm(split, desc=f"Processing {split_name} samples", colour="green"):
            ...
            try:
                ...
                logger.error(f"Error reading annotation file {label_path}: {str(e)}")
                skipped_images += 1
                continue
            ...
            try:
                ...
                logger.debug(f"Successfully read image {image_name} with shape {image.shape}")
            except Exception as e:
            ...
            for obj in label.get("objects", []):
                ...
                logger.debug(f"Processing object: class={class_name}, description={description}")
                
                if class_name == "claim ID":
                    if not check_for_valid_claim_id(description):
                        logger.debug(f"Skipping invalid claim ID: {description}")
                        skipped_objects += 1
                        continue
                    
                if class_name == "unknown":
                    logger.debug(f"Skipping whole image due to 'unknown' class")
            ...
            if not do_we_need_to_skip_the_whole_image:
                ...
                processed_images += 1
                logger.debug(f"Added full image {image_name} with {valid_objects} valid objects")
            else:
                skipped_images += 1
            ...
            for obj in label.get("objects", []):
                ...
                if class_name == "claim ID" and not check_for_valid_claim_id(description):
                    logger.debug(f"Skipping invalid claim ID crop: {description}")
                    continue
                ...
                try:
                    ...
                    if (bbox[0] >= bbox[2] or bbox[1] >= bbox[3] or
                        ...
                        logger.warning(f"Invalid bbox for {image_name}: {bbox}")
                        continue
                        
                    crop = image[int(bbox[1]):int(bbox[3]), int(bbox[0]):int(bbox[2])]
                    
                    if crop.size == 0:
                        logger.warning(f"Empty crop for {image_name} with bbox {bbox}")
                        continue
                    ...
                    crop_count += 1
                    logger.debug(f"Created crop for {class_name} with description '{description}'")
                    
                except Exception as e:
                    logger.error(f"Error creating crop from {image_name}: {str(e)}")
        ...
        logger.info(f"  - Total samples (full images + crops): {len(all_samples)}")

        # Write annotations for this split
        annotations_path = os.path.join(dataset_root, split_name, "annotations.jsonl")
        ...
        with open(annotations_path, "w") as f_out:
            for sample in tqdm(
                ...
                try:
                    ...
                    sample.pop("image")  # Remove image data before JSON serialization
                    f_out.write(json.dumps(sample) + "\n")
                    samples_written += 1
                    count += 1
                except Exception as e:
        ...
        logger.info(f"Successfully wrote {samples_written} samples to {split_name} split")
